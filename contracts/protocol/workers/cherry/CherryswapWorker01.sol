...
import "../../apis/cherry/ICherryMasterChef.sol";
...
/// @title CherryswapWorker01 is a CherryswapWorker with with reinvest-optimized and beneficial vault buyback functionalities
contract CherryswapWorker01 is OwnableUpgradeSafe, ReentrancyGuardUpgradeSafe, IWorker02 {
  /// @notice Libraries
  ...
  ICherryMasterChef public masterChef;
  ICherryFactory public factory;
  ICherryRouter02 public router;
  ICherryPair public override lpToken;
  ...
  /// @notice Configuration varaibles for Cherryswap
  uint256 public fee;
  uint256 public feeDenom;

  /// @notice Upgraded State Variables for CherryswapWorker01
  ...
  function initialize(
    ...
    lpToken = ICherryPair(address(_lpToken));
    address token0 = lpToken.token0();
    address token1 = lpToken.token1();
    farmingToken = token0 == baseToken ? token1 : token0;
    cherry = address(masterChef.cherry());
    ...
    // 6. Set Cherryswap swap fees
    fee = 9975;
    feeDenom = 10000;

    // 7. Check if critical parameters are config properly
    require(baseToken != cherry, "CherryswapWorker01::initialize:: base token cannot be a reward token");
    require(
      reinvestBountyBps <= maxReinvestBountyBps,
      "CherryswapWorker01::initialize:: reinvestBountyBps exceeded maxReinvestBountyBps"
    );
    require(
      (farmingToken == lpToken.token0() || farmingToken == lpToken.token1()) &&
        (baseToken == lpToken.token0() || baseToken == lpToken.token1()),
      "CherryswapWorker01::initialize:: LP underlying not match with farm & base token"
    );
    require(
      reinvestPath[0] == cherry && reinvestPath[reinvestPath.length - 1] == baseToken,
      "CherryswapWorker01::initialize:: reinvestPath must start with CHE, end with BTOKEN"
    );
  }

  /// @dev Require that the caller must be an EOA account to avoid flash loans.
  modifier onlyEOA() {
    require(msg.sender == tx.origin, "CherryswapWorker01::onlyEOA:: not eoa");
    _;
  }

  /// @dev Require that the caller must be the operator.
  modifier onlyOperator() {
    require(msg.sender == operator, "CherryswapWorker01::onlyOperator:: not operator");
    _;
  }

  //// @dev Require that the caller must be ok reinvestor.
  modifier onlyReinvestor() {
    require(okReinvestors[msg.sender], "CherryswapWorker01::onlyReinvestor:: not reinvestor");
    _;
  }
  ...
  function _reinvest(
    ...
    require(_treasuryAccount != address(0), "CherryswapWorker01::_reinvest:: bad treasury account");
    // 1. Withdraw all the rewards. Return if reward <= _reinvestThreshold.
    masterChef.withdraw(pid, 0);
    uint256 reward = cherry.balanceOf(address(this));
    if (reward <= _reinvestThreshold) return;

    // 2. Approve tokens
    cherry.safeApprove(address(router), uint256(-1));
    ...
    if (bounty > 0) {
      ...
      cherry.safeTransfer(_treasuryAccount, bounty.sub(beneficialVaultBounty));
    }
    ...
    // 7. Reset approval
    cherry.safeApprove(address(router), 0);
  ...
  function work(
    ...
    require(okStrats[strat], "CherryswapWorker01::work:: unapproved work strategy");
    require(
      lpToken.transfer(strat, lpToken.balanceOf(address(this))),
      "CherryswapWorker01::work:: unable to transfer lp to strat"
    );
  ...
  function getMktSellAmount(
    ...
    if (aIn == 0) return 0;
    require(rIn > 0 && rOut > 0, "CherryswapWorker01::getMktSellAmount:: bad reserve values");
  ...
  function _addShare(uint256 id) internal {
    ...
    if (balance > 0) {
      ...
      // 3. Deposit balance to CherryMasterChef
  ...
  function setReinvestConfig(
    ...
    require(
      ...
      "CherryswapWorker01::setReinvestConfig:: _reinvestBountyBps exceeded maxReinvestBountyBps"
    );
    require(_reinvestPath.length >= 2, "CherryswapWorker01::setReinvestConfig:: _reinvestPath length must >= 2");
    require(
      _reinvestPath[0] == cherry && _reinvestPath[_reinvestPath.length - 1] == baseToken,
      "CherryswapWorker01::setReinvestConfig:: _reinvestPath must start with CHE, end with BTOKEN"
    );
  ...
  function setMaxReinvestBountyBps(uint256 _maxReinvestBountyBps) external onlyOwner {
    require(
      ...
      "CherryswapWorker01::setMaxReinvestBountyBps:: _maxReinvestBountyBps lower than reinvestBountyBps"
    );
    require(
      _maxReinvestBountyBps <= 3000,
      "CherryswapWorker01::setMaxReinvestBountyBps:: _maxReinvestBountyBps exceeded 30%"
    );
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    require(rewardPath.length >= 2, "CherryswapWorker01::setRewardPath:: rewardPath length must be >= 2");
    require(
      rewardPath[0] == cherry && rewardPath[rewardPath.length - 1] == beneficialVault.token(),
      "CherryswapWorker01::setRewardPath:: rewardPath must start with CHE and end with beneficialVault token"
    );
  ...
  function setTreasuryConfig(address _treasuryAccount, uint256 _treasuryBountyBps) external onlyOwner {
    require(
      ...
      "CherryswapWorker01::setTreasuryConfig:: _treasuryBountyBps exceeded maxReinvestBountyBps"
    );
  ...
  function setBeneficialVaultConfig(
    ...
    require(
      ...
      "CherryswapWorker01::setBeneficialVaultConfig:: _beneficialVaultBountyBps exceeds 100%"
    );
    require(_rewardPath.length >= 2, "CherryswapWorker01::setBeneficialVaultConfig:: rewardPath length must >= 2");
    require(
      _rewardPath[0] == cherry && _rewardPath[_rewardPath.length - 1] == _beneficialVault.token(),
      "CherryswapWorker01::setBeneficialVaultConfig:: rewardPath must start with CHE, end with beneficialVault token"
    );
